      ...
      {[...Array(10)].map((_, i) => (
        <motion.div
          ...
          animate={{ 
            ...
            backgroundColor: i / 10 < load / 100 ? (load > 80 ? "#ef4444" : "#4F46E5") : "#ffffff"
          }}
    ...
    <div className="space-y-6 pb-12">
      ...
      >
        <div className="relative z-10">
          ...
          <h1 className="text-4xl font-black tracking-tighter mb-4 text-white">Global Edge Nodes</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {nodeData.map((node, i) => (
          <motion.div
            ...
            className="glass-subtle p-8 rounded-[2rem] border border-white/5 group hover:border-flow-indigo/20 transition-all relative overflow-hidden"
          >
            <div className="flex justify-between items-start mb-8">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-white/[0.03] flex items-center justify-center text-flow-indigo border border-white/5 group-hover:scale-110 transition-transform">
                  {node.region === "Asia" ? <Zap size={24} /> : <Server size={24} />}
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">{node.name}</h3>
                  <span className="text-[10px] font-black text-data-slate uppercase tracking-widest">{node.id} • {node.region}</span>
                </div>
              </div>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex justify-between text-[10px] font-black text-data-slate uppercase mb-3 tracking-widest">
                   <span>Node Load</span>
                   <span className={node.load > 80 ? "text-red-500" : "text-white"}>{node.load.toFixed(0)}%</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                 <div>
                    <div className="text-[8px] font-black text-data-slate uppercase mb-1">Latency</div>
                    <div className="text-sm font-mono font-bold text-insight-teal">{node.latency.toFixed(0)}ms</div>
                 </div>
                 <div>
                    <div className="text-[8px] font-black text-data-slate uppercase mb-1">Up-link</div>
                    <div className="text-sm font-mono font-bold text-flow-indigo italic">Secure</div>
                 </div>
              </div>
            </div>

            <button className="absolute bottom-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity p-2 text-data-slate hover:text-white">
