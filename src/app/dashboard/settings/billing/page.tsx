...
import { CreditCard, Check, ArrowRight, Loader2, Receipt, Zap, ShieldCheck, X, Sparkles, Lock, AlertCircle } from "lucide-react";
...
export default function BillingSettingsPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      {/* Header */}
      <div className="flex items-center gap-4 mb-10 text-sm font-bold uppercase tracking-widest text-left">
        ...
        <h1 className="text-[var(--text-primary)]">Billing & Subscription</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          ...
          >
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-10 text-left">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)] font-bold uppercase tracking-widest mt-1">Current Active License</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-10 text-left">
                 <div className="space-y-4">
                    <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Included Capabilities</div>
                 ...
                 <div className="space-y-4">
                    <div className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Billing Cycle</div>
                    <div className="p-6 rounded-2xl bg-[var(--surface-elevated)] border border-[var(--glass-border)]">
                       <div className="text-2xl font-black text-[var(--text-primary)]">$49.00 <span className="text-xs text-[var(--text-muted)] font-medium">/ month</span></div>
                       <p className="text-[10px] text-[var(--text-muted)] uppercase mt-2 font-black">Next renewal: May 12, 2026</p>
                    </div>
                 </div>
              </div>
              ...
              <div className="pt-8 border-t border-[var(--glass-border)] flex gap-4">
                 ...
                 <button className="px-8 py-4 rounded-xl glass border border-[var(--glass-border)] text-[var(--text-muted)] font-black text-xs uppercase tracking-widest hover:bg-[var(--text-primary)]/5 transition-all">Cancel Subscription</button>
              </div>
            </div>
          ...
          >
             <div className="flex items-center gap-4 mb-8">
                <Receipt className="text-[var(--text-muted)]" size={20} />
             ...
             <div className="space-y-4 text-left">
                {[
                  ...
                  <div key={inv.id} className="p-4 rounded-xl bg-white/[0.02] border border-[var(--glass-border)] flex items-center justify-between group hover:bg-[var(--surface-elevated)] transition-colors">
                     <div className="flex items-center gap-6">
                        <span className="text-xs font-bold text-[var(--text-primary)]">{inv.id}</span>
                        <span className="text-xs text-[var(--text-muted)] font-medium">{inv.date}</span>
                     </div>
                     <div className="flex items-center gap-8">
                        ...
                        <button className="p-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors opacity-0 group-hover:opacity-100">
                           <ArrowRight size={14} />
                        </button>
                     </div>
                  </div>
        ...
        <div className="lg:col-span-4 space-y-6 text-left">
           ...
           <motion.div 
            ...
            className="glass rounded-[2.5rem] p-10 border border-[var(--glass-border)]"
          >
             <h4 className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-8">Settlement Method</h4>
             ...
             <button 
              ...
              className="w-full mt-10 py-3 rounded-xl border border-[var(--glass-border)] text-[var(--text-muted)] text-[10px] font-black uppercase tracking-widest hover:bg-[var(--text-primary)]/5 transition-all"
             >
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div 
            ...
            className="glass w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-[3rem] border border-[var(--glass-border)] relative z-10 p-12 custom-scrollbar"
          >
            <button 
              onClick={() => setShowUpgradeModal(false)}
            ...
            <div className="text-center mb-12">
              <h2 className="text-4xl font-black tracking-tighter mb-4 text-[var(--text-primary)]">Upgrade Your Node</h2>
              <p className="text-[var(--text-muted)] font-bold uppercase tracking-widest text-xs">Select a higher transmission capacity</p>
              
              <div className="flex items-center justify-center gap-4 mt-8">
                <span className={`text-xs font-black uppercase tracking-widest ${billingCycle === "monthly" ? "text-flow-indigo" : "text-[var(--text-muted)]"}`}>Monthly</span>
                <button 
                  onClick={() => setBillingCycle(billingCycle === "monthly" ? "yearly" : "monthly")}
                  className="w-12 h-6 rounded-full bg-[var(--surface-elevated)] border border-[var(--glass-border)] relative p-1"
                >
                  <motion.div 
                ...
                <span className={`text-xs font-black uppercase tracking-widest ${billingCycle === "yearly" ? "text-flow-indigo" : "text-[var(--text-muted)]"}`}>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 text-left">
          ...
          <motion.div 
            ...
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.9, y: 20 }}
            className="glass w-full max-w-lg rounded-[3rem] border border-[var(--glass-border)] relative z-10 p-10"
          >
            <button 
              onClick={() => setShowPaymentModal(false)}
              className="absolute top-8 right-8 p-2 hover:bg-white/5 rounded-full transition-colors"
            ...
            <div className="mb-8">
              <h2 className="text-2xl font-black tracking-tight mb-2 text-[var(--text-primary)]">Update Settlement</h2>
              <p className="text-[var(--text-muted)] font-bold uppercase tracking-widest text-[10px]">Securely update your payment method</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-center gap-4 text-[8px] text-[var(--text-muted)] font-black uppercase tracking-[0.2em]">
