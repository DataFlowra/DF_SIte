    ...
    <div className="space-y-6 pb-12">
      ...
      >
        <div className="relative z-10 text-left">
          ...
          <h1 className="text-4xl font-black tracking-tighter mb-4 text-[var(--text-primary)]">Neural Control Center</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
           <div className="glass rounded-[2rem] p-8 border border-[var(--glass-border)]">
              <div className="flex items-center gap-4 mb-8">
                 ...
                 <h3 className="text-xl font-bold text-[var(--text-primary)]">Parameter Tuning</h3>
              </div>
              ...
              <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                    <div className="space-y-4">
                       <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] block ml-1">Inference Model</label>
                       <select 
                         value={config.model}
                         onChange={(e) => setFormData({...config, model: e.target.value})}
                         className="w-full bg-[var(--surface-elevated)] border border-[var(--glass-border)] rounded-xl px-4 py-3 text-sm font-medium outline-none focus:border-flow-indigo/50 transition-all text-[var(--text-primary)]"
                       >
                    ...
                    <div className="space-y-4">
                       <label className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] block ml-1">Stream Concurrency</label>
                       <div className="flex items-center gap-4">
                          <input 
                            type="number" 
                            value={config.concurrency}
                            onChange={(e) => setFormData({...config, concurrency: parseInt(e.target.value)})}
                            className="flex-1 bg-[var(--surface-elevated)] border border-[var(--glass-border)] rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-flow-indigo/50 transition-all text-[var(--text-primary)]"
                          />
                          <span className="text-[10px] text-[var(--text-muted)] font-black uppercase">Nodes</span>
                       </div>
                    </div>
                 </div>

                 <div className="pt-6 border-t border-[var(--glass-border)] flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-6">
                       <div className="flex items-center gap-3">
                          ...
                          />
                          <span className="text-xs font-medium text-[var(--text-muted)]">Edge Caching</span>
                       </div>
           ...
           <AnimatePresence>
             {status && (
               <motion.div 
                 ...
                 animate={{ opacity: 1, y: 0 }}
                 exit={{ opacity: 0 }}
                 className="p-6 rounded-2xl bg-insight-teal/10 border border-insight-teal/20 flex items-center gap-4 text-left"
               >
        ...
        <div className="lg:col-span-4 space-y-6 text-left">
           <div className="glass rounded-[2rem] p-8 border border-[var(--glass-border)] h-full flex flex-col">
              <h4 className="text-xs font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-8">Node Distribution</h4>
              ...
              <div className="mt-10 p-4 rounded-xl bg-white/[0.02] border border-[var(--glass-border)] flex items-center gap-4">
                 <Database className="w-4 h-4 text-flow-indigo" />
                 <div className="text-[10px] font-mono text-[var(--text-muted)]">STABLE_CLUSTER_VER: 0x9AF2</div>
              </div>
           </div>
        </div>
      </div>
